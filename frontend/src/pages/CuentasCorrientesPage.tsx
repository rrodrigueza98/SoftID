...
import type { CuentaCorriente, MovimientoCuentaCorriente, Tercero } from '../lib/types';
...
export default function CuentasCorrientesPage() {
  ...
  const { data: terceros, isLoading: loadingTerceros } = useQuery({
    queryKey: ['terceros', { empresaId, tipo: 'CLIENTE', search: debouncedSearch }],
    queryFn: async () =>
      (
        await api.get<Tercero[]>('/terceros', {
          params: { empresaId, tipo: 'CLIENTE', search: debouncedSearch || undefined },
        })
      ).data,
  });
  ...
  return (
    <div className="flex flex-col gap-6">
      <h1 className="text-xl font-semibold text-ink-900">Cuentas corrientes</h1>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[320px_1fr]">
        ...
        {!seleccionado ? (
          <Card className="flex h-64 items-center justify-center">
            <p className="text-sm text-ink-400">Elegí un cliente para ver su cuenta corriente.</p>
          </Card>
        ) : (
          <div className="flex flex-col gap-6">
            <Card className="p-5">
              <div className="flex items-start justify-between">
                ...
                <Button onClick={() => setReciboDialogOpen(true)}>Registrar cobro</Button>
              </div>
      ...
      {seleccionado && (
        <ReciboFormDialog open={reciboDialogOpen} onClose={() => setReciboDialogOpen(false)} tercero={seleccionado} />
      )}
    </div>
  );
}
